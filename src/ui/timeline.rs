use eframe::egui::{
    Align2, Area, Color32, CursorIcon, FontId, Frame, Id, Key, Pos2, Rect, Sense, Stroke,
...
impl TimelinePanel {
    ...
    fn loop_region(
        ...
        let lx1 =
            rect.left() + (self.loop_end * fps_atual() + 0.5) * box_width - self.scroll * box_width;
        let near = 6.0;
    ...
    fn marker_region(
        ...
        for (i, m) in self.markers.iter().enumerate() {
            let xs =
                rect.left() + (m.start * fps_atual() + 0.5) * box_width - self.scroll * box_width;

            if m.kind == MarkerType::Range {
                let xe =
                    rect.left() + (m.end * fps_atual() + 0.5) * box_width - self.scroll * box_width;
    ...
    pub fn show(&mut self, ui: &mut Ui, loop_active: bool, playing: bool) {
        ...
        if over {
            ...
            let hdelta = if scroll_delta.x.abs() > scroll_delta.y.abs() {
                scroll_delta.x
            } else {
                scroll_delta.y
            };
            ...
            if alt && hdelta != 0.0 {
                ...
                let f_cursor = (anchor.x - rect.left()) / bw0 + self.scroll;
                self.zoom = new_zoom;
                let bw1 = (40.0 * self.zoom).max(8.0);
                self.scroll = (f_cursor - (anchor.x - rect.left()) / bw1).max(0.0);
            } else if hdelta != 0.0 {
        ...
        self.display_frame += (self.current_frame as f32 - self.display_frame) * 0.25;
        ...
        if response.hovered() && ui.ctx().input(|i| i.key_pressed(Key::F)) {
            self.scroll = (self.display_frame + 0.5 - rect.width() / (2.0 * box_width)).max(0.0);
        }
        ...
        if playing && !self.scrollbar_drag {
            ...
            let vis_max = self.scroll + rect.width() / box_width - pad;
            if self.display_frame < vis_min {
                self.scroll = (self.display_frame - pad).max(0.0);
            } else if self.display_frame > vis_max {
                self.scroll = (self.display_frame + pad - rect.width() / box_width).max(0.0);
            }
        }

        // Frames visíveis considerando o scroll
        let first_visible = ((self.scroll - 1.5).floor()).max(0.0) as i32;
        let last_visible = (self.scroll + rect.width() / box_width + 0.5).ceil() as i32;

        // Atalho para converter um frame em x de tela (com scroll)
        // Captura apenas a cópia local de scroll, não self.
        let scroll = self.scroll;
        let x_of = |f: f32| -> f32 { rect.left() + (f + 0.5) * box_width - scroll * box_width };
        ...
        if loop_active && self.loop_end > self.loop_start {
            ...
            painter.rect_filled(
                Rect::from_min_max(Pos2::new(lx0, rect.top()), Pos2::new(lx1, ruler_bottom)),
                0.0,
        ...
        for f in first_visible..=last_visible {
            ...
            painter.line_segment(
                [Pos2::new(x, rect.top()), Pos2::new(x, y_bot)],
                Stroke::new(1.0, cor),
            );
        ...
        if loop_active && self.loop_end > self.loop_start {
            for &lx in &[
                x_of(self.loop_start * fps_atual()),
                x_of(self.loop_end * fps_atual()),
            ] {
                painter.line_segment(
                    [Pos2::new(lx, rect.top()), Pos2::new(lx, ruler_bottom)],
        ...
        for m in &self.markers {
            ...
            let selected = self.selected.map_or(false, |id| id == m.id);
            ...
            if m.kind == MarkerType::Range {
                ...
                painter.rect_stroke(
                    r,
                    3.0,
                    Stroke::new(if selected { 2.0 } else { 1.0 }, c),
                    StrokeKind::Inside,
                );
                painter.text(
                    r.center(),
                ...
                painter.line_segment(
                    [Pos2::new(xs, ruler_bottom), Pos2::new(xs, marker_bottom)],
        ...
        if let Some(start_sec) = self.creating_marker {
            if let Some(p) = pointer {
                ...
                let end_sec =
                    snap(((p.x - rect.left()) / box_width + self.scroll - 0.5) / fps_atual());
                if (end_sec - start_sec).abs() >= 0.5 {
                    ...
                    painter.rect_filled(
                        ...
                        3.0,
                        Color32::from_rgba_unmultiplied(90, 140, 255, 80),
                    );
                    painter.rect_stroke(
                        Rect::from_min_max(
                            Pos2::new(xs, ruler_bottom + 1.0),
        ...
        painter.line_segment(
            ...
            Stroke::new(1.0, Color32::from_rgba_unmultiplied(80, 220, 120, 70)),
        );
        ...
        let thumb_w = (rect.width() * (vis / content_frames).clamp(0.05, 1.0)).max(20.0);
        ...
        let thumb_x = rect.left() + sb_frac * (rect.width() - thumb_w);

        // trilho
        painter.rect_filled(scrollbar_rect, 0.0, Color32::from_rgb(30, 30, 38));
        // thumb
        painter.rect_filled(
            Rect::from_min_max(
        ...
        let cursor = if let Some(state) = self.loop_drag {
            match state.kind {
                LoopDrag::Start | LoopDrag::End => Some(CursorIcon::ResizeHorizontal),
            ...
            if scrollbar_rect.contains(pos) {
                ...
                // faixa de marcadores
                if let Some((_, kind)) =
                    self.marker_region(pos, rect, box_width, ruler_bottom, marker_bottom)
                {
                    match kind {
                ...
                match self.loop_region(pos, rect, box_width, ruler_bottom, loop_active) {
        ...
        if let Some(pos) = pointer {
            if pos.y >= ruler_bottom
                ...
                && rect.contains(pos)
                && self
                    .marker_region(pos, rect, box_width, ruler_bottom, marker_bottom)
                    .is_none()
            {
                let sz = Vec2::splat(18.0);
                // ponta do lápis (canto inferior‑esquerdo do SVG, 0,18) exatamente no cursor
                let r = Rect::from_min_max(pos - Vec2::new(0.0, sz.y), pos + Vec2::new(sz.x, 0.0));
                egui::Image::new(egui::include_image!("icons/lapis.svg")).paint_at(ui, r);
            }
        }
        ...
        if response.clicked() {
            if let Some(pos) = ui.ctx().pointer_interact_pos() {
                if pos.y >= keyframes_top && pos.y <= keyframes_bottom {
                    let f = ((pos.x - rect.left()) / box_width + self.scroll - 0.5)
                        .round()
                        .max(0.0) as u32;
            ...
            if let Some(pos) = ui.ctx().pointer_interact_pos() {
                if pos.y >= ruler_bottom && pos.y <= marker_bottom {
                        ...
                        self.marker_region(pos, rect, box_width, ruler_bottom, marker_bottom)
                    {
                        ...
                        let snap = |s: f32| (s * 2.0).round() / 2.0;
                        let sec = snap(
                            (((pos.x - rect.left()) / box_width + self.scroll - 0.5).max(0.0))
                                / fps_atual(),
                        );
                        let m = Marker {
        ...
        if response.drag_started() {
            if let Some(origin) = ui.ctx().input(|i| i.pointer.press_origin()) {
                if scrollbar_rect.contains(origin) {
                    ...
                    self.marker_region(origin, rect, box_width, ruler_bottom, marker_bottom)
                {
                    self.created_by_click = None;
                    ...
                    self.marker_drag = Some(MarkerDrag {
                        id: m.id,
                        kind,
                        sec0: ((origin.x - rect.left()) / box_width + self.scroll) / fps_atual(),
                        start0: m.start,
                        end0: m.end,
                    });
                    self.selected = Some(m.id);
                } else if origin.y >= ruler_bottom && origin.y <= marker_bottom {
                    self.created_by_click = None;
                    // Arrasto em área vazia da faixa de marcadores:
                    // inicia a criação de um novo Range
                    let snap = |s: f32| (s * 2.0).round() / 2.0;
                    let start_sec = snap(
                        (((origin.x - rect.left()) / box_width + self.scroll - 0.5) / fps_atual())
                            .max(0.0),
                    );
                    self.creating_marker = Some(start_sec);
                } else if origin.y >= keyframes_top && origin.y <= keyframes_bottom {
                    self.dragging_frame = true;
                } else if self.loop_drag.is_none() {
                    if let Some(kind) =
                        self.loop_region(origin, rect, box_width, ruler_bottom, loop_active)
                    {
                        self.loop_drag = Some(LoopDragState {
                            kind,
                            sec0: ((origin.x - rect.left()) / box_width + self.scroll)
                                / fps_atual(),
        ...
        if response.dragged() {
            if let Some(pos) = ui.ctx().pointer_interact_pos() {
                if self.scrollbar_drag {
                    ...
                    let frac = ((pos.x - rect.left() - thumb_w / 2.0) / track_w).clamp(0.0, 1.0);
                    self.scroll = frac * max_scroll;
                } else if let Some(drag) = self.marker_drag {
                    // Arrasta marcador
                    if let Some(m) = self.markers.iter_mut().find(|m| m.id == drag.id) {
                        let sec = ((pos.x - rect.left()) / box_width + self.scroll) / fps_atual();
                        ...
                        match drag.kind {
                            ...
                            MarkerDragKind::Start => {
                                m.start = snap((drag.start0 + delta).clamp(0.0, m.end - min_w));
                            }
                            MarkerDragKind::End => {
                                m.end = snap((drag.end0 + delta).max(m.start + min_w));
                            }
                        }
                    ...
                    let drag = self.loop_drag.map(|s| (s.kind, s.sec0, s.start0, s.end0));
                    if let Some((kind, sec0, start0, end0)) = drag {
                        ...
                        match kind {
                            ...
                            LoopDrag::Start => {
                                self.loop_start = (start0 + delta).clamp(0.0, end0 - min_w).round();
                            }
                            LoopDrag::End => {
                                self.loop_end = (end0 + delta).max(start0 + min_w).round();
                            }
                        }
                    }
                }
        ...
        if response.drag_stopped() {
            ...
            if let Some(start) = self.creating_marker {
                if let Some(pos) = ui.ctx().pointer_interact_pos() {
                    let snap = |s: f32| (s * 2.0).round() / 2.0;
                    let end = snap(
                        (((pos.x - rect.left()) / box_width + self.scroll) / fps_atual()).max(0.0),
                    );
        ...
        if let Some(edit_id) = self.editing_marker {
            ...
            let popup_pos = if let Some(m) = self.markers.iter().find(|m| m.id == edit_id) {
                let mid = (m.start + m.end) / 2.0;
                let cx = rect.left() + (mid + 0.5) * box_width - self.scroll * box_width;
            ...
            let popup_open = Area::new(popup_id)
                .fixed_pos(popup_pos)
                .show(ui.ctx(), |ui| {
                    Frame::popup(ui.style()).corner_radius(8).show(ui, |ui| {
                        ui.set_min_width(160.0);

                        if let Some(m) = self.markers.iter_mut().find(|m| m.id == edit_id) {
                            // rename
                            ui.horizontal(|ui| {
                                ui.label("Nome:");
                                if ui
                                    .add(TextEdit::singleline(&mut m.name).desired_width(100.0))
                                    .changed()
                                {
                                    self.markers_modificados = true;
                                }
                            ...
                            ui.horizontal_wrapped(|ui| {
                                for (_ic, &c) in MarkerColor::ALL.iter().enumerate() {
                                    ...
                                    let rect = Frame::NONE
                                        ...
                                        .rect;
                                    if ui
                                        .interact(rect, ui.next_auto_id(), Sense::click())
                                        .clicked()
                                    {
                                        m.color = c;
                                        self.markers_modificados = true;
            ...
            if !response.double_clicked()
                ...
                && !popup_open
                    .response
                    .rect
                    .contains(ui.ctx().pointer_interact_pos().unwrap_or(Pos2::ZERO))
            {
                self.editing_marker = None;
            }
