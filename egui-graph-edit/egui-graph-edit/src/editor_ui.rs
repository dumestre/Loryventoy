    ...
    CategoryType: CategoryTrait,
{
    ...
    pub fn draw_graph_editor_with_options(
        ...
        // Scroll / touchpad: pan by default; zoom only with Alt held
        if ui.rect_contains_pointer(clip_rect) && options.interactions_enabled {
            let scroll = ui.input(|i| i.smooth_scroll_delta());
            let alt = ui.ctx().input(|i| i.modifiers.alt);
            if alt && scroll.y != 0.0 {
                let zoom_delta = (-scroll.y * 0.002).exp();
                if let Some(cursor) = ui.ctx().input(|i| i.pointer.hover_pos()) {
                    self.pan_zoom.zoom_around(clip_rect, ui.style(), zoom_delta, cursor);
                } else {
                    self.zoom(ui, zoom_delta);
                }
            } else if scroll != egui::Vec2::ZERO {
                // Don't pan if the pointer is over a node's content area,
                // so DragValue fields can consume horizontal scroll.
                let pointer_over_node = ui.ctx().input(|i| i.pointer.hover_pos())
                    .map(|pos| {
                        self.cached_node_rects.values().any(|r| r.contains(pos))
                    })
                    .unwrap_or(false);
                if !pointer_over_node {
                    self.pan_zoom.pan += scroll;
                }
            }
        }
    ...
    fn draw_graph_editor_inside_zoom(
        ...
        if let Some(ref mut node_finder) = self.node_finder {
            let mut node_finder_area =
                Area::new(ui.id().with("node_finder")).order(Order::Foreground);
        ...
        for (input, output) in self.graph.iter_connections() {
            ...
            // Culling: skip connection if either node is off-screen (ports not recorded)
        ...
        // Cache node rects for next frame's scroll-pan check
        self.cached_node_rects = node_rects.clone();
        ...
        if interactions_enabled && mouse.any_pressed() && !cursor_in_finder {
    ...
    DataType: DataTypeTrait<UserState>,
{
    ...
    fn show_graph_node(
        ...
        if ui.visuals().dark_mode {
            ...
            title_text_color = color_from_hex("#222222").unwrap();
        } else {
        ...
        child_ui.vertical(|ui| {
            ...
            title_row_height = ui.min_size().y;
            ui.add_space(margin.y);

            // Content before inputs/outputs (e.g. Layer header)
            responses.extend(self.graph[self.node_id].user_data.content_before_outputs(
                ui,
                self.node_id,
                self.graph,
                user_state,
        ...
        let node_color = self.graph[self.node_id]
            .user_data
        ...
        let titlebar_height = title_row_height + 3.0 * margin.y;

        let (shape, outline) = {
            let rounding_radius = (8.0 * pan_zoom.zoom) as u8;
            ...
            let outline = if self.selected || outline_color.is_some() {
                let (fill, width) = if self.selected {
                    (Color32::from_rgb(180, 50, 50), 2.0)
                } else {
                    (
        ...
        let title_center_y = margin.y + title_row_height / 2.0;

        if can_flip && Self::flip_button(pan_zoom, ui, outer_rect, title_center_y).clicked() {
        ...
        if can_delete && Self::close_button(pan_zoom, ui, outer_rect, title_center_y).clicked() {
    ...
    fn close_button(pan_zoom: &PanZoom, ui: &mut Ui, node_rect: Rect, center_y: f32) -> Response {
        // Measurements
        let margin = 8.0 * pan_zoom.zoom;
        let size = 10.0 * pan_zoom.zoom;
        let stroke_width = 2.0;
        let offs_x = margin + size / 2.0;

        let position = pos2(node_rect.right() - offs_x, node_rect.top() + center_y);
    ...
    fn flip_button(pan_zoom: &PanZoom, ui: &mut Ui, node_rect: Rect, center_y: f32) -> Response {
        // Measurements
        let margin = 8.0 * pan_zoom.zoom;
        let size = 10.0 * pan_zoom.zoom;
        let stroke_width = 2.0;
        ...
        let position = pos2(node_rect.right() - offs_x * 2.0 - 4.0, node_rect.top() + center_y);
        let rect = Rect::from_center_size(position, vec2(size, size));
        let resp = ui.allocate_rect(rect, Sense::click());
